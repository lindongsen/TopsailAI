...
func (at *AutoTrigger) Start() {
	logger.Info("auto-trigger task started", "interval", at.interval.String(), "timeout", at.timeout.String())
	go at.run()
}
...
func (at *AutoTrigger) checkAllGroups() {
	...
	if err := at.db.Where("deleted_at IS NULL").Find(&groups).Error; err != nil {
		logger.Error("failed to fetch groups for auto-trigger", "error", err)
		return
	}

	for i := range groups {
		group := &groups[i]
		if err := at.checkGroup(ctx, group); err != nil {
			logger.Error("failed to check group for auto-trigger",
...
func (at *AutoTrigger) checkGroup(ctx context.Context, group *models.Group) error {
	...
	defer func() {
		if err := lockKey.Release(); err != nil {
			logger.Error("failed to release auto-trigger lock", "group_id", group.GroupID, "error", err)
		}
	}()
	...
	if err != nil {
		return fmt.Errorf("failed to evaluate auto-trigger: %w", err)
	}
	...
	if err := at.db.Model(&models.AgentMessageProcessing{}).
		...
		logger.Error("failed to check existing processing", "error", err)
	}
	if existingCount > 0 {
		logger.Info("auto-trigger already pending for message", "group_id", group.GroupID, "message_id", lastMessage.MessageID)
		return nil
	}
	...
	if err := at.db.Create(pendingMsg).Error; err != nil {
		return fmt.Errorf("failed to create pending message: %w", err)
	}
	...
	if err := at.db.Create(processingRecord).Error; err != nil {
		logger.Error("failed to create processing record", "error", err)
	}
	...
	if err := at.publisher.PublishAutoTriggerPendingMessage(group.GroupID, pendingMsg, triggerData); err != nil {
		return fmt.Errorf("failed to publish auto-trigger pending message: %w", err)
	}

	logger.Info("auto-trigger published pending message",
		"group_id", group.GroupID,
