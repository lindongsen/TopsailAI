...
|----------|---------|-------------|
| `ACS_HTTP_HOST` | `` (empty) | HTTP server listen host/IP. Empty (default) binds all interfaces (`0.0.0.0`). Set to `127.0.0.1` or `127.1.0.1` to bind a specific loopback address so multiple instances can run on the same machine. |
