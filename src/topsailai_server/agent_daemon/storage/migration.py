...
class DatabaseMigrator:
    ...
    def migrate(self):
        ...
        if 'api_key' in existing_tables:
            self._migrate_api_key_table()

        if 'api_key_session' in existing_tables:
            self._migrate_api_key_session_table()
    ...
    def _migrate_api_key_table(self):
        """Migrate the api_key table to add any missing columns."""
        logger.info("Checking api_key table for schema updates")
        ...
        for col_name, col_type in expected_columns.items():
            self._add_column_if_not_exists('api_key', col_name, col_type)

        self._create_index_if_not_exists('api_key', 'ix_api_key_api_key', 'api_key')
        self._create_index_if_not_exists('api_key', 'ix_api_key_role', 'role')

    def _migrate_api_key_session_table(self):
        """Migrate the api_key_session table to add any missing columns."""
        logger.info("Checking api_key_session table for schema updates")
        ...
        for col_name, col_type in expected_columns.items():
            self._add_column_if_not_exists('api_key_session', col_name, col_type)

        self._create_index_if_not_exists('api_key_session', 'ix_api_key_session_session_id', 'session_id')

    def _migrate_rate_limit_log_table(self):
        """Migrate the rate_limit_log table to add any missing columns."""
        logger.info("Checking rate_limit_log table for schema updates")
        ...
        for col_name, col_type in expected_columns.items():
            self._add_column_if_not_exists('rate_limit_log', col_name, col_type)
