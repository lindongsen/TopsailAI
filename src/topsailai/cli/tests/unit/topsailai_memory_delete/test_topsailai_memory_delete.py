...
class TestArguments:
    ...
    def test_valid_options(self):
        """Parse a title, workspace, yes, and json options."""
        args = cli.parse_args(
            ["My_Memory", "--workspace", "/memory", "--yes", "--json"]
        )
...
class TestMain:
    ...
    def test_deletes_with_yes_flag(self, capsys):
        ...
        with patch.object(
            ...
            code = cli.main(["--workspace", "/memory", "--yes", "Mem"])
        assert code == 0
    ...
    def test_aborts_when_unconfirmed(self, capsys):
        ...
        with patch.object(
            ...
            code = cli.main(["--workspace", "/memory", "Mem"])
        assert code == 0
    ...
    def test_not_found_returns_error(self, capsys):
        ...
        with patch.object(cli, "resolve_memory_file", return_value=None):
            code = cli.main(["--workspace", "/memory", "--yes", "Missing"])
