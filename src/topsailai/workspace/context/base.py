...
class ContextRuntimeBase(object):
    ...
    def _log_summarize_message_identity_changes(
        ...
        """
        Log messages in ``new_messages`` that are not identical (by ``is``) to any
        message in ``original_messages``.

        This helps observe whether summarization unexpectedly replaces or inserts
        messages. The summary message itself is excluded because it is always
        newly generated by the LLM and therefore never identical to an original
        message.

        This check is controlled by the ``TOPSAILAI_LOG_SUMMARIZE_MESSAGE_IDENTITY_CHANGES``
        environment variable and defaults to enabled.

        Args:
            method_name (str): Name of the summarization method for the log prefix.
            original_messages (list): Messages before summarization.
            new_messages (list): Messages after summarization.
            summary_message: The LLM-generated summary message to exclude.
        """
        if not env_tool.EnvReaderInstance.check_bool(
            "TOPSAILAI_LOG_SUMMARIZE_MESSAGE_IDENTITY_CHANGES", default=True
        ):
            return

        if not original_messages or not new_messages:
            return

        original_ids = {id(msg) for msg in original_messages}
        for i, msg in enumerate(new_messages):
            if msg is summary_message:
                continue
            if id(msg) not in original_ids:
                logger.warning(
                    "[%s] new_messages[%s] is not identical to any original message; "
                    "it may have been inserted or replaced during summarization",
                    method_name, i,
                )
