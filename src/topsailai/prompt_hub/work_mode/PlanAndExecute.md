...
1. `task`: The user will submit a task. If the user does not submit a task or the task is ambiguous, please initiate an [inquiry] to the user. The user will reply with the task.
2. `PlanAnalysis`: Analyze the task description to understand the task objectives, contextual information, and any constraints. If the task is ambiguous, initiate an [inquiry] to the user.
...
- If you "do not understand" the task objective, the output must include [inquiry]; otherwise, the output must include one and only one [action] or [final_answer].
