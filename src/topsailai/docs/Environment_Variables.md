...
| `TOPSAILAI_LLM_NON_RETRYABLE_BAD_REQUEST_MARKERS` | `no tool call found;function_call_output;tool_call_id` | Case-insensitive markers, separated by `;`, that extend the built-in list. When a `BadRequestError` message contains any marker, the request is treated as a deterministic request-shape error and raised immediately instead of consuming the retry budget. Built-in markers are always active and cannot be disabled. |
