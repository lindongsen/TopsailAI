...
| `TOPSAILAI_AGENT_ROLE` | `""` | Agent role, default is `worker`. Supported values: `manager`, `worker`. Ignored by the `topsailai_agent_chats` and `topsailai_team` entry points, which always pass role `manager` explicitly. |
...
| `TOPSAILAI_PRINT_STEP_MODE` | `"normal"` | Controls Agent2LLM step console detail. `normal` preserves legacy full output; `simple` prints bounded one-line summaries; each tool call additionally prints its first argument as `name=value` (single line, whitespace collapsed, 80-character preview, values of sensitive argument names such as password/token/secret/api_key masked as `***`, tool-call IDs never shown), but fully prints `raw_text` for steps whose names start with `task`, `thought`, `action`, `final`, or `inquiry`. Empty or invalid values fall back to `normal`. Can be inspected and switched at runtime with the `/print_step_mode` instruction (numbered list, `*` marks the current value). |
...
| `TOPSAILAI_LLM_NON_RETRYABLE_BAD_REQUEST_MARKERS` | `""` | Optional case-insensitive substring markers separated by `;`. Configured markers extend non-disable-able built-in contextual rules for known orphan tool-call request errors; any configured marker match makes a `BadRequestError` fail immediately instead of consuming the retry budget. Leave empty unless a provider-specific deterministic request error is known. |
...
| `TOPSAILAI_CONTEXT_MESSAGES_HEAD_OFFSET_TO_KEEP` | `0` | Number of head messages to retain after summarization. Also acts as the lower bound for tail-window expansion, so the tail window can never grow into the summarized region. |
| `TOPSAILAI_CONTEXT_MESSAGES_TAIL_OFFSET_TO_KEEP` | `0` | Number of tail messages to retain after summarization. The most recent messages are preserved in addition to the head offset and the last user message. The window is tool-call-pair atomic: it is expanded backwards so it never starts with an orphaned tool result, which may keep one extra assistant message carrying `tool_calls`. |
...
| `TOPSAILAI_AGENT2LLM_SUMMARY_TOKEN_RESERVE` | `4096` | Non-negative token budget reserved for the future Agent2LLM summary during feasibility checks. Negative or invalid values fall back to `4096`; `0` disables the reserve. |
...
| `TOPSAILAI_REALTIME_TOKEN_CALCULATION` | `0` | When `1`, unspecified token-count calls calculate the current runtime messages; when `0` (default), they use cached `TokenStat.current_tokens`. Token-threshold safety checks always count their explicit pending message set. |
...
- `TOPSAILAI_AGENT2LLM_TOKEN_SUMMARIZE_THRESHOLD` is checked by `ContextRuntimeAgent2LLM.is_need_summarize_for_processing()`. When the estimated tokens of the current pending Agent2LLM messages exceed this threshold, summarization is triggered in addition to the existing message-count check. Setting this variable to `0` disables the token-based check.
- `TOPSAILAI_USER2AGENT_TOKEN_SUMMARIZE_THRESHOLD` is checked by `ContextRuntimeData.is_need_summarize_for_processed()`. When the estimated tokens of the current pending User2Agent messages exceed this threshold, summarization is triggered in addition to the existing message-count check. The default is `0`, which disables the token-based check.
- `TOPSAILAI_REALTIME_TOKEN_CALCULATION` controls `_get_current_tokens()` only when no message collection is passed: `1` calculates the current runtime messages and `0` (default) uses cached `TokenStat.current_tokens`. Calls that pass an explicit message collection always calculate that collection, so token-threshold safety reflects pending messages added after the previous LLM response. This variable is read on each applicable call to `_get_current_tokens()`.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `TOPSAILAI_HUMAN_DECISION_TIMEOUT` | `0` | Default integer seconds to wait for a decision from the `ask_decision` tool. `0` = infinite, but only honored when an interactive TTY is present; otherwise returns `unavailable` immediately. Positive integers cap the wait; invalid or fractional values fall back to `0`. |
